...
@section('content')
    <nav aria-label="breadcrumb" style="background: #f8f5ef">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page"style="font-size: 160%;">Report</li>
        </ol>
    </nav>
    ...
    <div id="myDiv">
        ...
        <div class="centered"> <!-- Wrap the centered part in a div with the "centered" class -->
            ...
            <br>
            <div>Following Requests/Grievances addressed to the President of India are being forwarded for <strong>{{$name}}</strong> during the peroid from <strong>{{$date_from}}</strong> To <strong> {{$date_to}}</strong> for appropriate attention. Kindly expedite disposal/status report to the petitioner:
            </div>
        </div>
        <br>
        <div class="centered">
            ...
            @foreach ($applications as $application)
                <tr>
                    ...
                    <td>{{$application->reason ? $application->reason->reason_desc : ''}}</td>
                </tr>
            @endforeach
            </tbody>
            </table>
