...
@section('content')
        ...
        <div class="col-md-4" style="text-align: right;">
            <input class="form-control" type="text" placeholder="Search" id="searchInput"data-bs-toggle="modal" data-bs-target="#exampleModal"/>
                <label class="form-check-label">
                    <input type="radio" name="draft" value="Y" checked>
                    Draft
                </label>
                <label class="form-check-label">
                    <input type="radio" name="Pending" value="N" >
                    Pending with DH
                </label>
                <label class="form-check-label">
    ...
    <div class="row"id="pageContent">
        @if(Auth::user()->roles->first()->pivot->role_id == 1 && $source!='search')
            @foreach ($applications->where('created_by', Auth::user()->id)->filter(function ($application) {
                    return $application->statuses()->whereIn('status_id', [1, 4, 5])->first();
                }) as $application)
                <div class="col col-md-4" style="padding: 20px;">
                    <div class="card shadow">
                        ...
                        <div class="card-footer text-body-secondary">
                            @if( $application->statuses->first()->pivot->status_id < 6 && $application->statuses->first()->pivot->status_id != 5)
                            ...
                            @if (auth()->check() && auth()->user()->roles->pluck('id')->contains(1) &&
                                $application->statuses->first() && $application->statuses->first()->pivot->status_id == 4 && $application->reply=='')
                                <div class="float-end">
                                    <form action="{{ route('applications.show', ['application' => $application]) }}" method="GET">
...
{{--        @elseif(Auth::user()->roles->first()->pivot->role_id == 2)--}}
{{--            @foreach ($applications->where('created_by', Auth::user()->id)->filter(function ($application) {--}}
{{--            return $application->statuses()->whereIn('status_id', [1, 4, 5])->first();--}}
{{--        }) as $application)--}}
...
{{--        @elseif(Auth::user()->roles->first()->pivot->role_id == 3 && $source!='search')--}}
{{--            @foreach ($applications->where('created_by', Auth::user()->id)->filter(function ($application) {--}}
{{--                    return $application->statuses()->whereIn('status_id', [1, 4, 5])->first();--}}
{{--                }) as $application)--}}
{{--                @yield('application_list')--}}
